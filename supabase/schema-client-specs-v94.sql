-- ASB Intranet V94 - Clients / CDC + Planning de règlement indépendant
...
-- Ancienne table V93 conservée pour compatibilité, mais le nouveau planning est indépendant.
...
-- Nouveau module indépendant : Planning de règlement
...
create policy "client_specs_authenticated_all" on public.client_specs for all to authenticated using (true) with check (true);
create policy "client_spec_items_authenticated_all" on public.client_spec_items for all to authenticated using (true) with check (true);
create policy "client_spec_payment_terms_authenticated_all" on public.client_spec_payment_terms for all to authenticated using (true) with check (true);
create policy "client_payment_schedules_authenticated_all" on public.client_payment_schedules for all to authenticated using (true) with check (true);
create policy "client_payment_schedule_items_authenticated_all" on public.client_payment_schedule_items for all to authenticated using (true) with check (true);
...
create index if not exists client_payment_schedules_project_id_idx on public.client_payment_schedules(project_id);
create index if not exists client_payment_schedule_items_schedule_id_idx on public.client_payment_schedule_items(schedule_id);

-- Bucket Storage pour les visuels du module CDC.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('client-specs', 'client-specs', true, 10485760, array['image/jpeg','image/png','image/webp'])
...
create policy "client_specs_images_read" on storage.objects for select to authenticated using (bucket_id = 'client-specs');
create policy "client_specs_images_insert" on storage.objects for insert to authenticated with check (bucket_id = 'client-specs');
create policy "client_specs_images_update" on storage.objects for update to authenticated using (bucket_id = 'client-specs') with check (bucket_id = 'client-specs');
create policy "client_specs_images_delete" on storage.objects for delete to authenticated using (bucket_id = 'client-specs');
