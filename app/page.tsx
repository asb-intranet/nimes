...
function Projects({ projects, photos, docs, notes, employees, links, refreshAll }: any) {
  ...
  async function archiveProject(p: any) {
    const ok = confirm(`Archiver le chantier "${p.name}" ?`);
    if (!ok) return;
  ...
  async function deleteProject(p: any) {
    const ok = confirm(
      `ATTENTION : supprimer définitivement le chantier "${p.name}" ?\n\nLes photos, documents, notes et demandes liés à ce chantier seront supprimés de la base.`
    );
  ...
  return (
    <div>
      <Section
        ...
        subtitle="Chaque chantier possède sa fiche avec photos, documents, notes et demandes liées."
      />
      ...
      <Card>
        <form onSubmit={saveProject} className="grid gap-3 md:grid-cols-3">
          ...
          <Field label="Client">
            <Input value={form.client} onChange={(e: any) => setForm({ ...form, client: e.target.value })} />
          </Field>
          ...
          <Field label="Couleur">
            <Input type="color" value={form.color} onChange={(e: any) => setForm({ ...form, color: e.target.value })} />
          </Field>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[400px_1fr]">
        <div className="space-y-4">
          ...
          {activeProjects.map((p: any) => (
            <Card
              key={p.id}
              className={`border-l-8 ${current?.id === p.id ? "ring-2 ring-slate-900" : ""}`}
              style={{ borderLeftColor: p.color || "#0f172a" }}
            >
              <div className="flex items-start justify-between gap-3">
                <div>
              ...
              <p className="mt-3 text-xs text-slate-500">
                Photos : <b>{photos.filter((x: any) => x.project_id === p.id).length}</b> ·
                Documents : <b>{docs.filter((x: any) => x.project_id === p.id).length}</b> ·
                Notes : <b>{notes.filter((x: any) => x.project_id === p.id).length}</b>
              </p>

              <div className="mt-3 grid grid-cols-2 gap-2">
                <Button variant="primary" onClick={() => setSelectedId(p.id)}>
                  Accéder
                </Button>

                <Button variant="secondary" onClick={() => editProject(p)}>
                  Modifier
                </Button>

                <Button variant="amber" onClick={() => archiveProject(p)}>
                  Archiver
                </Button>

                <Button variant="danger" onClick={() => deleteProject(p)}>
                  Supprimer
                </Button>
              </div>
            </Card>
          ...
          {activeProjects.length === 0 && (
            <Card>
              <p className="text-sm text-slate-500">Aucun chantier actif.</p>
            </Card>
          )}
          ...
          {archivedProjects.map((p: any) => (
            <Card
              key={p.id}
              className="border-l-8 opacity-75"
              style={{ borderLeftColor: p.color || "#0f172a" }}
            >
              <div className="flex items-start justify-between gap-3">
                <div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                <Button variant="green" onClick={() => restoreProject(p)}>
                  Réactiver
                </Button>

                <Button variant="danger" className="col-span-2" onClick={() => deleteProject(p)}>
          ...
          {archivedProjects.length === 0 && (
            <Card>
              <p className="text-sm text-slate-500">Aucun chantier archivé.</p>
            </Card>
          )}
        </div>
...
function ProjectDetail({ project, photos, docs, notes, employees, links, refreshAll }: any) {
  ...
  const assignedEmployees = links
    .filter((l: any) => l.project_id === project.id)
    .map((l: any) => employees.find((e: any) => e.id === l.employee_id))
    .filter(Boolean);
  ...
  async function deleteDoc(doc: any) {
    const ok = confirm(`Supprimer le document "${doc.name}" ?`);
    if (!ok) return;
      ...
      <Card className="border-l-8" style={{ borderLeftColor: project.color || "#0f172a" }}>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">{project.client || "Client non renseigné"}</p>
        ...
        {project.description && (
          <p className="mt-4 rounded-2xl bg-slate-50 p-3 text-sm text-slate-700">{project.description}</p>
        )}

        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl bg-slate-50 p-3">
            <p className="text-xs font-bold uppercase text-slate-500">Photos</p>
            <p className="text-2xl font-black">{projectPhotos.length}</p>
          </div>
          <div className="rounded-2xl bg-slate-50 p-3">
            <p className="text-xs font-bold uppercase text-slate-500">Documents</p>
            <p className="text-2xl font-black">{projectDocs.length}</p>
          </div>
          <div className="rounded-2xl bg-slate-50 p-3">
            <p className="text-xs font-bold uppercase text-slate-500">Notes</p>
            <p className="text-2xl font-black">{projectNotes.length}</p>
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="flex flex-wrap gap-2">
          {assignedEmployees.map((e: any) => (
            <span key={e.id} className="rounded-full bg-slate-100 px-3 py-2 text-sm font-bold">
              {e.firstname} {e.lastname} <span className="text-slate-500">· {e.position || e.role}</span>
            </span>
          ))}

          {assignedEmployees.length === 0 && (
            <p className="text-sm text-slate-500">Aucun salarié affecté à ce chantier.</p>
          )}
        </div>
      </Card>
              ...
              <div key={p.id} className="overflow-hidden rounded-2xl border bg-slate-50">
                ...
                <div className="space-y-2 p-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => deletePhoto(p)} className="flex items-center justify-center rounded-xl bg-red-600 px-2 py-1 text-xs font-bold text-white">
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
            ...
            {projectDocs.map((d: any) => (
              <div key={d.id} className="rounded-2xl border bg-slate-50 p-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex gap-2">
                    <a href={d.file_url} target="_blank" className="rounded-xl border bg-white px-3 py-2 text-xs font-bold">
                      Lire
                    </a>
                    <button type="button" onClick={() => deleteDoc(d)} className="rounded-xl bg-red-600 px-3 py-2 text-xs font-bold text-white">
                      Supprimer
                    </button>
                  </div>
                </div>
...
function Employees({ employees, projects, refreshAll }: any) {
  ...
  async function removeAssignment(assignment: any) {
    const ok = confirm("Retirer cette affectation ?");
    if (!ok) return;
  ...
  return (
    <div>
      <Section title="Gestion salariés" subtitle="Création salarié, affectation aux chantiers et préparation planning salarié." />

      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <h3 className="mb-4 font-black">Créer salarié</h3>
          ...
          <form onSubmit={addEmployee} className="space-y-3">
            ...
            <Field label="Rôle">
              <Select value={form.role} onChange={(e: any) => setForm({ ...form, role: e.target.value })}>
                ...
                <option value="chef">Chef chantier</option>
        ...
        <Card>
          ...
          <form onSubmit={assign} className="space-y-3">
            <Field label="Salarié">
              <Select value={employeeId} onChange={(e: any) => setEmployeeId(e.target.value)}>
                <option value="">Choisir salarié</option>
                {employees.map((e: any) => (
                  <option key={e.id} value={e.id}>
                    {e.firstname} {e.lastname} — {e.position || e.role}
                  </option>
                ))}
              </Select>
            </Field>
          ...
          <div className="mt-6 space-y-2">
            ...
            {assignments.map((a: any) => (
              <div key={a.id} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 p-3 text-sm">
                <span>
                  <b>{employeeName(a.employee_id)}</b> → {projectNameLocal(a.project_id)}
                </span>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {employees.map((e: any) => {
          ...
          return (
            <Card key={e.id}>
              <h3 className="font-black">{e.firstname} {e.lastname}</h3>
              <p className="text-sm text-slate-500">{e.position}</p>
              <Badge>{e.role}</Badge>

              <div className="mt-4 space-y-1">
                <p className="text-xs font-bold uppercase text-slate-500">Chantiers affectés</p>

                {employeeAssignments.map((a: any) => (
                  <div key={a.id} className="rounded-xl bg-slate-50 px-3 py-2 text-xs font-bold">
                    {projectNameLocal(a.project_id)}
                  </div>
                ))}

                {employeeAssignments.length === 0 && (
                  <p className="text-xs text-slate-500">Aucun chantier</p>
                )}
              </div>
            </Card>
          );
