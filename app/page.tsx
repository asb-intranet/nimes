...
export default function Page() {
  ...
  async function refreshAll() {
    const [p, ph, d, e, l, n, v, r, pl] = await Promise.all([
...
function Planning({ projects, employees, links, planning, refreshAll }: any) {
  const [form, setForm] = useState({ project_id: "", employee_id: "", title: "", start_date: "", end_date: "", start_time: "", end_time: "", notes: "" });
  const assignedEmployees = form.project_id ? links.filter((l: any) => l.project_id === form.project_id).map((l: any) => employees.find((e: any) => e.id === l.employee_id)).filter(Boolean) : employees;
  ...
  async function addPlanning(e: any) {
    ...
    const { error } = await supabase.from("employee_planning").insert({ project_id: form.project_id, employee_id: form.employee_id, title: form.title, start_date: form.start_date, end_date: form.end_date || form.start_date, start_time: form.start_time || null, end_time: form.end_time || null, notes: form.notes });
    if (error) return alert(error.message);
    setForm({ project_id: form.project_id, employee_id: "", title: "", start_date: "", end_date: "", start_time: "", end_time: "", notes: "" });
    await refreshAll();
  }
  ...
  const sortedPlanning = [...planning].sort((a: any, b: any) => String(a.start_date || "").localeCompare(String(b.start_date || "")));

  return (
    <div>
      <Section title="Planning" subtitle="Planning lié aux chantiers et aux salariés affectés." />
      <Card>
        <form onSubmit={addPlanning} className="grid gap-3 md:grid-cols-3">
          <Field label="Chantier"><Select value={form.project_id} onChange={(e: any) => setForm({ ...form, project_id: e.target.value, employee_id: "" })}><option value="">Choisir chantier</option>{projects.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}</Select></Field>
          ...
          <div className="grid grid-cols-2 gap-2"><Field label="Heure début"><Input type="time" value={form.start_time} onChange={(e: any) => setForm({ ...form, start_time: e.target.value })} /></Field><Field label="Heure fin"><Input type="time" value={form.end_time} onChange={(e: any) => setForm({ ...form, end_time: e.target.value })} /></Field></div>
      ...
      <div className="mt-6 space-y-3">
        {sortedPlanning.map((item: any) => (
          <Card key={item.id}>
            <div className="flex items-start justify-between gap-3">
              <div><h3 className="font-black">{item.title}</h3><p className="text-sm text-slate-500">{projectNameLocal(item.project_id)} · {employeeName(item.employee_id)}</p><p className="mt-1 text-sm">{item.start_date}{item.end_date && item.end_date !== item.start_date ? ` → ${item.end_date}` : ""}{item.start_time ? ` · ${item.start_time}` : ""}{item.end_time ? ` - ${item.end_time}` : ""}</p>{item.notes && <p className="mt-2 rounded-2xl bg-slate-50 p-3 text-sm">{item.notes}</p>}</div>
              <Button variant="danger" onClick={() => deletePlanning(item)}>Supprimer</Button>
            </div>
          </Card>
        ))}
        {sortedPlanning.length === 0 && <Card><p className="text-sm text-slate-500">Aucune ligne de planning pour le moment.</p></Card>}
      </div>
    </div>
  );
}
...
function Vehicles({ vehicles, refreshAll }: any) {
  ...
  return (
    <div>
      <Section title="Gestion véhicules" subtitle="Parc véhicules, conducteur, km, entretien, CT et assurance." />
      <Card>
        <form onSubmit={addVehicle} className="grid gap-3 md:grid-cols-3">
          ...
          <Button className="md:col-span-3">Ajouter véhicule</Button>
        </form>
      </Card>
      <div className="mt-6 grid gap-4 md:grid-cols-3">{vehicles.map((v: any) => <Card key={v.id}><h3 className="font-black">{v.name}</h3><p className="text-sm text-slate-500">{v.plate}</p><p className="mt-2 text-sm">Conducteur : <b>{v.driver}</b></p><p className="text-sm">KM : <b>{v.km}</b></p><Badge tone={v.status === "probleme" ? "red" : v.status === "entretien" ? "amber" : "green"}>{v.status}</Badge></Card>)}</div>
    </div>
  );
}
