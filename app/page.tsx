...
function Management({ projects, photos = [], docs = [], notes = [], materials = [], vigilance = [], employees, planning, invoices, revenues, returns = [], companyExpenses = [], clientPayments = [], refreshAll }: any) {
  ...
  if (tab === "paiements") {
    ...
    return <div className="space-y-5">
      ...
      <Card><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-black">Liste des règlements clients</h2><Badge tone="blue">{visiblePayments.length}</Badge></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-xs uppercase text-slate-500"><th className="p-3">Date</th><th className="p-3">Client</th><th className="p-3">N° facture</th><th className="p-3 text-right">Montant TTC</th><th className="p-3">Moyen</th><th className="p-3 text-right">Actions</th></tr></thead><tbody>{visiblePayments.map((p: any) => <tr key={p.id} className={isClientPaymentPaid(p) ? "border-b bg-emerald-50 text-emerald-950" : "border-b"}><td className="p-3 font-bold">{p.payment_date || "—"}</td><td className="p-3">{p.client}</td><td className="p-3">{p.invoice_number || "—"}</td><td className="p-3 text-right font-black">{money(Number(p.amount_ttc || 0))}</td><td className="p-3">{p.payment_method || "—"}</td><td className="p-3"><div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => editPayment(p)}>Modifier</Button><Button variant="green" onClick={() => markPaymentPaid(p)}>{isClientPaymentPaid(p) ? "Payé ✓" : "Payé"}</Button><Button variant="danger" onClick={() => deletePayment(p)}>Supprimer</Button></div></td></tr>)}{visiblePayments.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-slate-500">Aucun règlement enregistré.</td></tr>}</tbody></table></div></Card>
    </div>;
  }
