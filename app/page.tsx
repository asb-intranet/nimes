...
function ClientSpecs({ specs, items, paymentTerms = [], paymentSchedules = [], paymentScheduleItems = [], projects, refreshAll }: any) {
  ...
  function exportSchedulePdf() {
    ...
    const rows = scheduleItems.map((i: any) => `<tr><td><b>${i.label || "Échéance"}</b><br/><span>${i.notes || ""}</span></td><td>${i.due_date ? formatDisplayDate(i.due_date) : (i.due_text || "À définir")}</td><td class="num">${Number(i.percentage || 0)}%</td><td class="num"><b>${money(i.amount_ttc || 0)}</b></td></tr>`).join("");
